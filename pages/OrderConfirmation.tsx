...
${order?.shippingInfo.city}${order?.shippingInfo.state ? ', ' + order?.shippingInfo.state : ''}
Morocco
...
ORDER ITEMS
-----------
${order?.items.map((item: any) => `${item.designTitle} - ${item.mockupType} (${item.size}, ${item.color}) x${item.quantity} - ${(item.price * item.quantity).toFixed(2)} DH`).join('\n')}

ORDER SUMMARY
-------------
Subtotal: ${order?.subtotal.toFixed(2)} DH
Shipping: ${order?.shipping.toFixed(2)} DH
Tax: ${order?.tax.toFixed(2)} DH
Total: ${order?.total.toFixed(2)} DH

Payment Method: ${order?.paymentInfo.method || 'Credit Card'}${order?.paymentInfo.cardLast4 ? ' - Card ending in ' + order?.paymentInfo.cardLast4 : ''}
...
Thank you for your order!
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8" id="order-ticket">
            ...
            <div>
              ...
              <div className="bg-gray-50 dark:bg-gray-750 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
                ...
                <div className="mt-4 pt-4 border-t border-gray-300 dark:border-gray-600">
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Payment Method: {order.paymentInfo.method || 'Credit Card'}
                    {order.paymentInfo.cardLast4 && (
                      <span className="font-mono font-semibold"> - Card ending in ****{order.paymentInfo.cardLast4}</span>
                    )}
                  </p>
                </div>
              </div>
